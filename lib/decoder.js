...
(function (__factory, __root) {
  ...
  Decoder.prototype.decode = function (code, cbWrite) {
    ...
    //console.log("dec: " + JSON.stringify(ast));
  ...
  Decoder.prototype._unescape = function (str) {
  ...
  Decoder.prototype._nextAst = function () {
    ...
    if (token === COMMON.T_LITERAL_1 ||
        token === COMMON.T_LITERAL_2 ||
        token === COMMON.T_LITERAL_3 ||
        token === COMMON.T_LITERAL_4) {
      return COMMON.astCreateNode(
        COMMON.T_LITERAL, this._readSizedLiteral(parseInt(token, 10)));
    } else if (token === COMMON.T_LITERAL_TABLE) {
      return COMMON.astCreateNode(
        COMMON.T_LITERAL, this._ids[this._get(2)]);
    } else if (token === COMMON.T_LITERAL_CODE) {
      return COMMON.astCreateNode(
        COMMON.T_LITERAL, this._readLiteralCode());
    } else if (token === COMMON.T_LITERAL) {
      return COMMON.astCreateNode(
        COMMON.T_LITERAL, this._unescape(this._getToEnd()));
    }
  ...
  Decoder.prototype._readTable = function () {
    ...
    var idx = 0;
    var id = this._unescape(this._getToEnd());
    while (id.length) {
      this._ids[COMMON.d2r2(idx++)] = id;
      id = this._unescape(this._getToEnd());
    }
  };
  ...
  Decoder.prototype._readSizedLiteral = function (len) {
    return this._unescape(this._get(len));
  };
  ...
  _map[COMMON.T_BREAK] = function (node) {
    this._cbWrite('break');
    var id = COMMON.astGetProp(node, COMMON.AST_LITERAL);
    if (id !== null) {
      this._cbWrite(' ');
      this._cbWrite(id);
    }
  ...
  _map[COMMON.T_DOT_MEMBER] = function (node) {
    this._next(COMMON.astGetProp(node, COMMON.AST_LEFT));
    this._cbWrite('.');
    this._next(COMMON.astGetProp(node, COMMON.AST_RIGHT));
  };

  _map[COMMON.T_ARRAY_MEMBER] = function (node) {
    this._next(COMMON.astGetProp(node, COMMON.AST_LEFT));
    this._cbWrite('[');
    this._next(COMMON.astGetProp(node, COMMON.AST_RIGHT));
    this._cbWrite(']');
