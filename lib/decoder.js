...
(function (__factory, __root) {
  ...
  Decoder.prototype._nextAst = function () {
    ...
    var isLitCode = false;
    if (token === COMMON.T_ID) {
      return COMMON.astCreateNode(COMMON.T_ID, this._readIdCode());
    } else if ("123456789".indexOf(token) >= 0) {
      return COMMON.astCreateNode(
        COMMON.T_ID, this._readSizedId(parseInt(token, 10)));
  ...
  Decoder.prototype._readId = function () {
    ...
    if (token === COMMON.T_ID) {
      ...
      return this._readSizedId(parseInt(token, 10));
    }
  };
  ...
  Decoder.prototype._readSizedId = function (len) {
