...
(function (__factory, __root) {
  ...
  Encoder.prototype._finalizeBlock = function () {
    ...
    if (keys.length) {
      ...
      for (i = 0, len = out.length; i < len; ++i) {
        this._cbWrite(COMMON.esc(out[i]));
  ...
  _acornMap["MemberExpression"] = function (node) {
    return COMMON.astCreateNode(COMMON.T_MEMBER,
  ...
  _acornMap["Identifier"] = function (node) {
    return COMMON.astCreateNode(COMMON.T_ID, node.name);
  };
  ...
  Encoder.prototype._next = function (node) {
    ...
    if (node[0] !== COMMON.T_LITERAL && node[0] !== COMMON.T_ID) {
  ...
  Encoder.prototype._writeId = function (val) {
    if (!val) {
      this._writeToken(COMMON.T_ID_CODE);
      this._writeToken(COMMON.T_END);
      return;
    }

    // TODO - empirical analysis
    //  2 is actually longer than the CODE version,
    //  but doesn't require a table entry...
    var escval = COMMON.esc(val);
    if (escval.length <= 2) {
      this._writeToken(COMMON.T_ID);
      this._code += escval;
      this._writeEnd();
      return;
    }

    if (!hasOwnProperty.call(this.ids, val)) {
      var t = COMMON.d2r2(this.nextId++);

      // Note: anywhere there can be an id, we can start a new block
      // TODO - empirical analysis 70 vs 70^2 vs 70^3 table size?
      if (t.length > 2) {
        this._finalizeBlock();
        t = COMMON.d2r2(this.nextId++);
      }
